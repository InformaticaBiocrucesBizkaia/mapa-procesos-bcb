        ...
        txt_subprocesos += 
            "<div class=\"contenedor contenedor_interno\" id=\"contenedor_internos_rrhh\">"+
                "<div class=\"bc_mp_internos_esquema\">"+
                    ...
                    "</div>";
                    mapaProcesos_contenido_procesos_nivel_ii_cont(jsonObj,i,j2);
    }
        txt_subprocesos +=
                "</div>"+
            "</div>";
...
//NIVEL: mp.i.jj
function mapaProcesos_contenido_procesos_nivel_ii_cont(jsonObj,cont1,cont2) {
    //Tenemos el JSON completo y el nivel en el que estamos
    var datos_mp_fullcontent = jsonObj;
    var i = cont1;
    var j = cont2;
    ...
    for (var k = 0; k < datos_mp_content.length; k++) {
        ...
        if ((k) < (datos_mp_content.length/2)){
            ...
            if (primera_vez_col1 == false){
                ...
                txt_subprocesos += 
                                "</div>"+
                                "<div class=\"bc_mp_division_2\">"+"&nbsp;"+"</div>"+
                            "</div>";
                txt_subprocesos +=
                        "</div>"+
                    "</div>";
        ...
        txt_subprocesos += 
            "<div class=\"bc_mp_division_2\">&nbsp;"+
                "<div class=\"contenedor contenedor_interno\" id=\"contenedor_internos_rrhh\">"+
                    "<div class=\"bc_mp_internos_esquema\">"+
                        "<div class=\"bc_mp_titulo\">"+
                            "<span class=\"class_esquema_titulo bc_mp_a\">"+ k + result_nombre_nivel1 +" -> " + result_nombre_nivel2 +" -> " + result_nombre +" "+ result_nivel+"</span>"+
                        "</div>"+
                        "<div class=\"bc_mp_division_2\">"+"&nbsp;";
        mapaProcesos_contenido_procesos_nivel_iii(jsonObj,i,j,k);
         
        txt_subprocesos += 
                        "</div>"+"<div class=\"bc_mp_division_2\">"+"&nbsp;"+"</div>"+
                    "</div>";
    }
